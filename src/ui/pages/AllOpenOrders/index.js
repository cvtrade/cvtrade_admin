...
const AllOpenOrders = () => {
    ...
    const statuslinkFollow = (row) => {
        return (
            <div>
                    <button className="btn btn-danger btn-sm me-2" onClick={() => { handleStatus(row?._id) }}>Cancel Order</button>
            </div>
        );
    };
    ...
    const columns = [
        ...
        { name: "Currency", selector: row => row.base_currency_short_name, },
    ...
    const handleStatus = async (id) => {
        await AuthService.OpenOrderStatus(id).then(async result => {
            if (result.success) {
                ...
                <main>
                    ...
                    <div className="container-xl px-4 mt-n10">
                        <div className="card mb-4">
                            ...
                            <DataTableBase columns={columns} data={dataToShow || []} pagination={false} />
                        </div>
